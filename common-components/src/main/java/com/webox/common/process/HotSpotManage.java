...
/*
    public Response loadHotSpotService(String userId) {
        Response response = new Response();
        List<HotSpot> hotspotlist = hotSpotRepository.findByUserId(userId);
        List<Service> serviceList = new ArrayList<Service>();
        hotspotlist.sort((a,b)->a.getRank().compareTo(b.getRank()));
       // userList.forEach(user->{user.getUserAccount().forEach(account->accountList.add(account));});
        hotspotlist.forEach(o->{
            if(o.getStatus()){
            Service service = serviceRepository.findByServiceId(o.getServiceId());
